...
function AddNewSeriesSearchResult({ event }: AddNewSeriesSearchResultProps) {
  const {
    tvdbId,
    titleSlug,
    title,
  ...
  const handleAddEventModalClose = useCallback(() => {
    setIsNewAddEventModalOpen(false);
  }, []);
  ...
  return (
    <div className={styles.searchResult}>
      ...
      <AddNewSeriesModal
        isOpen={isNewAddEventModalOpen && !isExistingSeries}
        event={event}
        initialSeriesType={seriesType}
        onModalClose={handleAddEventModalClose}
      />
    </div>
  );
}
